...
function classic_compat_css() {
	...
	if ( $settings->classic_use_compat_css ) {
		echo "<link rel='stylesheet' type='text/css' href='" . wptouch_get_bloginfo( 'template_directory' ) . "/compat.min.css?ver=" . wptouch_refreshed_files() . "' /> \n";		
	}
}
...
function classic_show_attached_image_filter( $content ) {
	...
	if ( $settings->classic_show_attached_image || $settings->classic_show_attached_image_on_page ) {
