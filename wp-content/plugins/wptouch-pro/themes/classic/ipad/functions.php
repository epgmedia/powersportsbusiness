...
function classic_ipad_init() {
	...
	if ( !is_admin() ) {
		...
		$minfile = WPTOUCH_DIR . '/themes/classic/ipad/ipad.min.js';

		if ( file_exists( $minfile ) ) {
			wp_enqueue_script( 'classic-ipad-js', wptouch_get_bloginfo('template_directory') . '/ipad.min.js', array( 'jquery-form' ), wptouch_refreshed_files() );
		} else {
			wp_enqueue_script( 'classic-ipad-js', wptouch_get_bloginfo('template_directory') . '/ipad.js', array( 'jquery-form' ), wptouch_refreshed_files() );
		}

		wp_enqueue_script( 'iscroll-js', wptouch_get_bloginfo('template_directory') . '/iscroll-lite.min.js', array( 'classic-ipad-js' ), wptouch_refreshed_files() );

		if ( show_webapp_notice() ) {
			$minfile = WPTOUCH_DIR . '/include/js/add2home.min.js';		
			if ( file_exists( $minfile ) ) {
				wp_enqueue_script( 'add2home', WPTOUCH_URL . '/include/js/add2home.min.js', array( 'iscroll-js' ), wptouch_refreshed_files() );
			} else {
				wp_enqueue_script( 'add2home', WPTOUCH_URL . '/include/js/add2home.js', array( 'iscroll-js' ), wptouch_refreshed_files() );
			}
		}
	} 
}
