...
abstract class Publishthis_API_Common {
	 ...
	 * Get content by hash
	 */
	function get_embed_content_by_hash( $hash, $token, $referer='' ) {
		$post_params = array ( 'token' => $token, 'hash' => $hash, 'referer' => urlencode($referer) );
		$params = array ( 'token' => $token, 'hash' => $hash );

		$url = $this->_compose_api_call_url( '/content/embed/documents', $params );

		try {
			$response = $this->_request ( $url, false, false, true, $post_params );
	...
	/**
	 * Get content by docid
	 */
	function get_content_by_docid( $docId, $token ) {
		$params = array ( 'token' => $token, 'extendedView' => true );

		$url = $this->_compose_api_call_url( '/content/document/'.$docId, $params );
$url = 'http://webapi.dev.publishthis.com/rest/content/document/'.$docId . '?' . http_build_query( $params );
//echo $url;
		try {
	...
	*/
	 ...
	 * http://docs.publishthis.com/edcenter/developers-and-admins/api-reference/api-method-mixesmixids/
	 */
	function get_feeds_by_ids( $feed_ids ) {
		...
		$url = $this->_compose_api_call_url( '/mixes/' . $ids, $params );
	...
	function get_feed_content_by_id( $feed_id, $params = array() ) {
		$params = $params + array ( 'results' => 10, 'token' => $this->_get_token() );

		$url = $this->_compose_api_call_url( '/content/automated/mix/'.$feed_id, $params );
	...
	function get_tweets_by_feed_id( $feed_id, $params = array() ) {
		...
		$url = $this->_compose_api_call_url( '/content/automated/tweets/mix/'.$feed_id, $params );
	...
	function get_custom_data_by_feed_id( $feed_id, $params = array() ) {
		...
		$url = $this->_compose_api_call_url( '/mixes/'.$feed_id.'/custom-data/', $params );
	...
	function get_feed_templates( $params = array() ) {
		...
		$url = $this->_compose_api_call_url( '/mixes/mix-templates/', $params );
	...
	function get_feeds_since_timestamp( $timestamp = 0, $template_id = 0, $params = array() ) {
		...
		$timestamp = date( "YmdTH:i:sZ", $timestamp );
		...
		do {
			$url = $this->_compose_api_call_url( '/mixes/since/'.$timestamp, $params );
  ...
  /***
    get Information about a Mix
    http://webapi.publishthis.com/rest/mixes/{mixIds}?token={your token api key}

    
  */
  function get_mix_information_by_id( $mix_id, $params = array() ) {
			$params = $params + array ( 'results' => 50, 'skip' => 0, 'token' => $this->_get_token() );
			...
			$url = $this->_compose_api_call_url( '/mixes/'.$mix_id, $params );
			...
			try {			
				...
				foreach ( $result_list as $mix ) {
					$_mix = array ( 'mixId' => $mix->mixId, 'displayName' => $mix->title, 'templateId' => $mix->templateId );
	...
	function get_section_content( $feed_id, $section_id, $params = array() ) {
		...
		do {
			$url = $this->_compose_api_call_url( '/content/curated/mix/'.$feed_id.'/section/'.$section_id, $params );
