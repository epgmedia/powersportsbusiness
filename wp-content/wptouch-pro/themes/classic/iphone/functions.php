...
function classic_init() {	
	...
	if ( $output_classic_scripts && !is_admin() ) {
		...
		if ( file_exists( $minfile ) ) {
			wp_enqueue_script( 'classic-js', wptouch_get_bloginfo('template_directory') . '/theme.min.js', array( 'jquery-form' ), wptouch_refreshed_files() );
		} else {
			wp_enqueue_script( 'classic-js', wptouch_get_bloginfo('template_directory') . '/theme.js', array( 'jquery-form' ), wptouch_refreshed_files() );
		}
