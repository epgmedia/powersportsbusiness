		...
		<div id="menu-tab5">
			...
			<?php } else { ?>
			 <h2><?php _e( "Account Login", "wptouch-pro" ); ?></h2>
				<form name="loginform" id="loginform" action="<?php wptouch_bloginfo('wpurl'); ?>/wp-login.php?redirect_to=<?php wptouch_the_current_page_url(); ?>" method="post" class="clearfix">
				<ul>
					<li class="inputs">
					...
					<?php if ( classic_show_account_tab() ) { ?>
						<li><a class="no-ajax" href="<?php wptouch_get_bloginfo( 'wpurl' ); ?>/wp-register.php"><?php _e( 'Register for an account', 'wptouch-pro' ); ?></a></li>
						<li><a class="no-ajax" href="<?php wptouch_get_bloginfo( 'wpurl' ); ?>/wp-login.php?action=lostpassword"><?php _e( 'Reset password', 'wptouch-pro' ); ?></a></li>
					<?php } ?>
				</ul>
				</form>	
			<?php } ?>
