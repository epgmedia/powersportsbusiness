...
/* Select & Checkbox toggles */
	...
	WPtouchCheckToggle( '#classic_webapp_use_loading_img', '#setting_classic_webapp_loading_img_location, #setting_classic_ipad_webapp_loading_img_location, #setting_webapp-copytext-info' );
	...
	jQuery( '#touchboard .box-holder' ).equalHeights( 275 , 400 );
	...
	if ( jQuery( '#wptouch-icon-list' ).length ) {
		...
		jQuery( 'ul.icon-menu input.checkbox' ).change( function() {
			switch( jQuery( this ).attr() ) {
				case 'checked':
					jQuery( this ).parent( 'li' ).find( '.checkbox' ).attr( 'checked', true );
					WPtouchDoTreeDisable();
					break;	
				default:
					jQuery( this ).parent( 'li' ).find( '.checkbox' ).attr( 'checked', false );
					WPtouchDoTreeDisable();
					break;	
			}
		});
...
function WPtouchTooltipSetup() {
	doBncTooltip( 'a.wptouch-tooltip', '#wptouch-tooltip', 8, -40, 'left' );
	
	/* Tooltips for items on the far right of the page */
	doBncTooltip( 'a.wptouch-tooltip-left', '#wptouch-tooltip-left', 8, -40, 'right' );
}
